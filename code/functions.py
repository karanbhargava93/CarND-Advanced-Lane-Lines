...
def thresholded_image(gray):
    ...
    combined[((gradx == 1) | (grady == 1)) & ((mag_binary == 1) | (dir_binary == 1))] = 1 # [((mag_binary == 1) & (dir_binary == 1))] = 1  #
...
class Line():
    ...
    def update(self, curvature, mid, pts, width):
        ...
        if ((self.first_frame == True) ):
            ...
            if((width > widthmin) & (width < widthmax)): # np.absolute(self.curvature - curvature) < 300.0) & 
                self.detected = True
                self.curvature = (1-theta)*self.curvature + theta*curvature
                self.pts = self.pts * (1-theta) + pts * theta
                self.offset = self.offset * (1-theta) + mid * theta

        else:
            # Checking that they have similar curvature and if true then proceed to change things
            # Also check if road is in between a certain width
            ...
            # sum_pts = np.sum(np.absolute(self.pts - pts))

            # if ((sum_pts < threshpts)):
            #     # Lanes detected
            #     self.detected = True
            #     self.curvature = (1-theta)*self.curvature + theta*curvature
            #     self.pts = self.pts * (1-theta) + pts * theta
    ...
    def process_image(self, img):
        ...
        combined = thresholded_image(gray)
        ...
        # points_left_avg  = np.average(left_fitx)
        # points_right_avg = np.average(right_fitx)
        ...
        trackwidth = np.absolute(x_left_pix - x_right_pix)

        trackwidth = xm_per_pix*trackwidth

        # print(trackwidth)

        # if ((trackwidth > 3) & (trackwidth < 4)):
        self.update(curvature, position_from_center, pts, trackwidth)



        # Draw the lane onto the warped blank image
        cv2.fillPoly(color_warp, np.int_([self.pts]), (0,255, 0))
        ...
        cv2.putText(result,'Radius of Curvature: %.2fm' % self.curvature,(20,40), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,255,255),2)

        if self.offset < 0:
            position = 'left'
        else:
            position = 'right'
        cv2.putText(result,'Distance From Center: %.2fm %s' % (np.absolute(self.offset), position),(20,80), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,255,255),2)
